...
module mkBF16Divider (BF16DividerIFC);
  ...
  // BUG FIX (same root cause as BF16ExpCore.mkBF16Exp / BF16Sigmoid.mkBF16Sigmoid,
  // and the source of this copy-paste: the comment above literally says
  // "exactly like sigmoid"): stage1 used to be written *only* by put_data,
  // which unconditionally set valid=True. Since a register holds its last
  // value when nothing writes it, once a single element was fed, stage1
  // stayed valid forever and kept re-processing the same stale element
  // every subsequent cycle with no real input. Fix: put_data only pulses a
  // wire; a separate rule runs every cycle and explicitly inserts a bubble
  // (valid=False) into stage1 when no new data arrived that cycle.
  RWire#(Tuple2#(Bit#(16), Bit#(16))) put_wire <- mkRWire;

  rule update_stage1;
    case (put_wire.wget()) matches
      tagged Valid {.a, .b}: stage1 <= PipelineStage{a: a, b: b, valid: True};
      tagged Invalid:        stage1 <= PipelineStage{a: 16'h0000, b: 16'h0000, valid: False};
    endcase
  endrule
  ...
  // BUG FIX: getResult() used to leave result_valid entirely to be cleared
  // by a future bubble reaching stage4 (which can take several cycles from
  // the last real input). This created a window where an already-consumed
  // result could still read as available, letting a caller collect a
  // stale result a second time -- especially risky right at an
  // operation-boundary hand-off. Fixed by pulsing a "consumed" wire from
  // getResult() and having pipeline_stage4 (the sole writer of
  // result_valid) clear it immediately when consumed and no fresh data is
  // arriving that same cycle, instead of waiting for an unrelated bubble.
