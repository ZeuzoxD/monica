#!/bin/sh
# BF16 divider streaming ablation - PPA driver. Mirrors run_ppa.sh 1:1.
#   ./run_ppa_divider.sh sim          -> build+run all 15 configs, print the cycles/op table
#   ./run_ppa_divider.sh verilog      -> emit one synthesizable .v per config into ppa/verilog/
#   ./run_ppa_divider.sh verbose      -> per-transaction logs for ALL configs -> ppa/logs/<cfg>.log
#   ./run_ppa_divider.sh verbose r8_n32 -> per-transaction log for ONE config, straight to stdout
#   ./run_ppa_divider.sh spot r4_n64  -> just the 12 manual-verification SPOT CHECK blocks
#   ./run_ppa_divider.sh              -> sim + verilog
set -e
cd "$(dirname "$0")"
...
do_sim () {
  ...
  for c in $CFGS; do
    ...
    echo "  -> bsc compiling mkTbDiv_$c ..."
    if ! bsc -sim -bdir ppa/obj -simdir ppa/obj -info-dir ppa/obj \
        -g mkTbDiv_$c -u Tb_BF16Divider_SIMD_AblationPPA.bsv > ppa/logs/build_$c.log 2>&1; then
    ...
    if ! bsc -sim -e mkTbDiv_$c -bdir ppa/obj -simdir ppa/obj -o ppa/sim_$c >> ppa/logs/build_$c.log 2>&1; then
  ...
  echo "  e.g.  ./run_ppa_divider.sh spot r4_n64"
}

do_verilog () {
  mkdir -p ppa/verilog ppa/vobj
  rm -f ppa/verilog/mkDividerAblation_*.v
  for d in $DUTS; do
    bsc -verilog -bdir ppa/vobj -vdir ppa/verilog -info-dir ppa/vobj \
        -g mkDividerAblation_$d -u BF16Divider_SIMD_AblationPPA.bsv > /dev/null 2>&1
  done
  echo ""
  echo "Verilog for synthesis in ppa/verilog/  (top module = mkDividerAblation_r<lanes>)"
  echo "Also copy FIFO2.v and RevertReg.v from \$BLUESPECDIR/Verilog/ before synth."
  for f in ppa/verilog/mkDividerAblation_*.v; do
...
build_verbose () {
  ...
  bsc -sim -D VERBOSE -bdir ppa/vobj_verbose -simdir ppa/vobj_verbose \
      -info-dir ppa/vobj_verbose -g mkTbDiv_$c -u Tb_BF16Divider_SIMD_AblationPPA.bsv > /dev/null 2>&1
  bsc -sim -e mkTbDiv_$c -bdir ppa/vobj_verbose -simdir ppa/vobj_verbose \
